...
if __name__ == "__main__":
    ...
    TARGET_POSITION = np.array([], dtype=int).reshape(0, 3)
    for i in range(DURATION * ENV.SIM_FREQ // 3):
        TARGET_POSITION = np.append(TARGET_POSITION, [[0, 0, 1.0]], axis=0)

    for i in range(DURATION * ENV.SIM_FREQ // 3):
        TARGET_POSITION = np.append(TARGET_POSITION, [[0, 1.0, 1.0]], axis=0)

    for i in range(DURATION * ENV.SIM_FREQ // 3):
        TARGET_POSITION = np.append(TARGET_POSITION, [[0, 1, 0.0]], axis=0)

    # TARGET_POSITION = np.array([])
    # for i in range(DURATION * ENV.SIM_FREQ):
    #     TARGET_POSITION = np.append(TARGET_POSITION, [0, 4.0*np.cos(0.006*i), 1.0])
